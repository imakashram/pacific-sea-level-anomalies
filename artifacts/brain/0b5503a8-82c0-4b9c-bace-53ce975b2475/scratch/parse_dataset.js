...
const csvPath = "artifacts/api-server/data/climate_change.csv";
...
for (let i = 1; i < lines.length; i++) {
  ...
  const indicator = parts[0].replace(/^"|"$/g, "").trim();
  const year = parseInt(parts[4].replace(/^"|"$/g, "").trim(), 10);
  const val = parseFloat(parts[5].replace(/^"|"$/g, "").trim());
